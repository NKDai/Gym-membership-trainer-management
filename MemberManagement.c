...
void mm_MemberManagement_AddingMember(MemberManager *mb_member_manager)
{
	...
	if(correct == 1)
	{	
		int key = mm_AddMember(mb_member_manager, new_member);
		if(key==1)
		{
...
void mm_MemberManagement_RemovingMember(MemberManager *mb_manager)
{
	...
	if(mb_IsAssigned(selector)==1)
	{
		Noti("You have to unassign trainer before remove!");
		return;
	}
	...
	if(selector!= NULL)
	{
		...
		if(action==1)
		{
			int member_index = mm_GetMemberIndex(mb_manager, id);
			if(member_index!=-1)
			{
				int key = mm_RemoveMember(mb_manager, member_index);
				if(key==1)
				{
